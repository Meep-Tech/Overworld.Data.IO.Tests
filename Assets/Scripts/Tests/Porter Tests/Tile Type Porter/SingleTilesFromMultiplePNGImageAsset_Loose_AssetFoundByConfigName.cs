...
namespace Overworld.Data.IO.Tests {
  public static class SingleTilesFromMultiplePNGImageAsset_Loose_AssetFoundByConfigName {
    ...
    public static readonly PorterTester<Tile.Type>.ImportWithConfigAndDummyFilesTest Test
      = new(
        "Single Tiles From Multiple PNG Image Assets -Loose -AssetFoundByConfigName",
        new[] {
          $"./{Path.GetFileNameWithoutExtension(TilePorterTestConstants.SecondSingleTilePngFileName)}.json",
        ...
        validateCreatedTypesAndProccessedFiles: (builtArchetypes, touchedFiles) => {
          ...
          else if (!touchedFiles.Any(tf => Path.GetFileName(tf) == $"{Path.GetFileNameWithoutExtension(TilePorterTestConstants.SecondSingleTilePngFileName)}.json")) {
            return new(false, $"Did not touch config file: {Path.GetFileNameWithoutExtension(TilePorterTestConstants.SecondSingleTilePngFileName)}.json");
          }
